...
public class Visitors {
    ...
    static abstract class BaseVisitor<T> extends HeroicQueryBaseVisitor<T> {
        @Override
    ...
    static class ExpressionVisitor extends BaseVisitor<Expression> {
        ...
        @Override
        public Expression visitExpressionPrecedence(final HeroicQueryParser.ExpressionPrecedenceContext ctx) {
        ...
        @Override
        public Expression visitSourceRangeAbsolute(HeroicQueryParser.SourceRangeAbsoluteContext ctx) {
            final Context c = context(ctx);
        ...
        @Override
        public Expression visitSourceRangeRelative(HeroicQueryParser.SourceRangeRelativeContext ctx) {
            final Context c = context(ctx);
        ...
        public Expression visitExpressionList(final HeroicQueryParser.ExpressionListContext ctx) {
            final Context c = context(ctx);
            final List<Expression> expressions = ctx.expr().stream().map(e -> e.accept(this))
                .collect(
                    Collectors.toList());
        ...
        public Expression visitQuery(final HeroicQueryParser.QueryContext ctx) {
            ...
            return new QueryExpression(c, aggregation, source, range, where,
                with.orElseGet(ImmutableMap::of),
                as.orElseGet(ImmutableMap::of));
        }

        @Override
        public Expression visitAggregationBy(final HeroicQueryParser.AggregationByContext ctx) {
            final Context c = context(ctx);

            final FunctionExpression left = ctx.expression.accept(this)
                .cast(FunctionExpression.class);
        ...
        @Override
        public Expression visitAggregationByAll(final HeroicQueryParser.AggregationByAllContext ctx) {
            final Context c = context(ctx);

            final FunctionExpression left = ctx.expr().accept(this)
                .cast(FunctionExpression.class);

            return new FunctionExpression(c, GROUP, ImmutableList.of(new EmptyExpression(c), left),
                ImmutableMap.of());
        }

        @Override
        public Expression visitAggregationPipe(final HeroicQueryParser.AggregationPipeContext ctx) {
            final Context c = context(ctx);

            final List<Expression> values = ctx.expr().stream().map(e -> e.accept(this))
                .collect(
                    Collectors.toList());
        ...
        @Override
        public Expression visitExpressionAll(final HeroicQueryParser.ExpressionAllContext ctx) {
            final Context c = context(ctx);
            return new FunctionExpression(c, EMPTY, ImmutableList.of(), ImmutableMap.of());
        }
    ...
    static class FilterVisitor extends BaseVisitor<Filter> {
        ...
        public Filter visitFilterIn(HeroicQueryParser.FilterInContext ctx) {
            ...
            final StringExpression key =
                ctx.key.accept(EXPRESSION).cast(StringExpression.class);
